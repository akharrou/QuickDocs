...
-	`lldb` gives you the option to set it *[`lldb`]* up to stop *(pause/halt/break/rest)* *[during execution [of your program]]*, at certain points/locations *(e.g. at a particular line *[in source code]*, a particular function)* – optionally adding conditions, so as to only stop at that point/location when the *[added]* condition is met *(fulfilled/reached)*. <br>
	...
	An analogy *(a situation/narrative same, or resembling, in essence)* *[for breakpoints]* would be that of a ***bus stops***, `lldb` being the, quote on quote, *"bus"*. *[In this analogy]*, **breakpoints** are the **bus stops** where **`lldb`**, the **bus**, would stop. The bus stops are known by **address** *(a particular line, in a *[source code]* file)* or by (a nearby) building/junction/area *(a particular function [in your code])*. The **conditions** are like the plans ahead of time of whether or not the bus should stop when it reaches the *bus stop* *(i.e checkpoint, i.e breakpoint)*, e.g.: "the *bus* only stops here *[at this bus stop]* **in the mornings**, **if it is a weekday**".
<!--
